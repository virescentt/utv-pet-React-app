...
## Хуки
Хуки это некоторые функции которые предоставляет Реакт. Они всегда начинаются со слова *use*. Хуки можно использовать в функциональных компонентах либо в своих собственных хуках. Стандартный всего 6 штук:
...
### Key attr
При создании списков обязательным условием является указание некоторого ключа, - **key** - который предназначен для хранения уникального значения. Как правило это *id* используемого объекта.
Пример:
```jsx
{posts.map((post) => 
...
// Для неуправляемых элементов. с помощью него может получить доступ к DOM элементу откуда уже вытянем value. Созданную ссылку передаем через атрибут ref={} к нужному элементу. У нее есть всего одно поле current, которое и представляет собой элемент. У него естественно есть нужное поле value (bodyInputRef.current.value). Хотя манипулировать ДОМ деревом не рекомендуется. Это топ, но для созданных компонентов допов надо будет указать React.forwardRef((props, ref)) и далее куда надо передать его ref={ref}.
