...
*Решение:* у нас уже есть единождый вызов функции `fetchPosts()` через `UseEffect()`, так что туда можно просто добавить зависимость от `page`, а из функции `changePost` вторую строчку вызова можно тогда вообще убрать.
...
const [fetchPosts, isPostsLoading, postError] = useFetching(async (limit, page) => {
  ...
  Также надо тогда добавить в нашем хуке `useFetching` прописать распаковку аргументов в коллбэк.
